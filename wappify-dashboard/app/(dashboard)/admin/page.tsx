...
export default async function AdminDashboardPage() {
  ...
  const activeSubs = organizations.filter(
    (o) => o.subscription?.status === "ACTIVE"
  ).length;
  const totalMessages = organizations.reduce(
    (acc, o) => acc + ((o as any).messagesSent || 0),
    0
  );
  const totalTokens = organizations.reduce(
    (acc, o) => acc + (o.geminiTokensUsed || 0),
    0
  );
                  ...
                  <td className="px-6 py-5 align-top">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold border border-indigo-200">
                        {org.name ? org.name[0].toUpperCase() : <Store className="w-5 h-5" />}
                      </div>
                      <div className="flex flex-col">
                        <span className="font-semibold text-foreground text-base tracking-tight">{org.name}</span>
                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                          <Mail className="w-3 h-3" />
                          {owner?.user?.email || "No owner"}
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-5 align-top">
                    <div className="flex flex-col gap-1.5">
                      <span className="font-medium text-foreground text-[14px]">
                        {org.products.length} products • {org.members.length} members
                      </span>
                      <div className="flex items-center gap-3 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1"><MessageSquare className="w-3 h-3" /> {(org as any).messagesSent || 0}</span>
                        <span className="flex items-center gap-1"><Sparkles className="w-3 h-3" /> {org.geminiTokensUsed}</span>
                        <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> Joined {new Date(org.createdAt).toLocaleDateString()}</span>
                      </div>
                    </div>
                  </td>
                    ...
                    {org.subscription ? (
                      <div className="flex flex-col gap-1">
                        <span className={`inline-flex w-fit items-center gap-1.5 rounded-md px-2 py-1 text-xs font-bold border uppercase tracking-wider ${
                          ...
                          <CheckCircle2 className={`w-3.5 h-3.5 ${org.subscription.status === 'ACTIVE' ? 'opacity-100' : 'opacity-50'}`} />
                          {org.subscription.planTier}
                        </span>
                        <span className="text-[10px] text-muted-foreground pl-1">
                          {org.subscription.paymentMethod} • {org.subscription.status}
                        </span>
                        {org.subscription.status === 'PENDING' && org.subscription.paymentMethod === 'UPI' && (
                          <span className="text-[10px] text-amber-600 pl-1 font-medium">Ref: {org.subscription.upiTransactionRef}</span>
                        )}
                        <SubscriptionActions subscriptionId={org.subscription.id} status={org.subscription.status} />
                      </div>
                    ) : (
                ...
                <tr>
                  <td colSpan={3} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-3 text-muted-foreground">
                      ...
                      <p className="font-medium">No organizations found across the platform.</p>
                      <p className="text-sm">When users create organizations, they will appear here automatically.</p>
                    </div>
                  </td>
                </tr>
              )}
