...
import type { ContactDetail } from "./ContactDetailDrawer";
...
export default function ContactList({ customers }: { customers: SerializedCustomer[] }) {
  const [search, setSearch] = useState("");
  const [sortBy, setSortBy] = useState<SortKey>("recent");
  const [selectedCustomer, setSelectedCustomer] = useState<ContactDetail | null>(null);
  ...
  const openContact = (c: SerializedCustomer) => {
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center rounded-xl border bg-card">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {search ? "Try a different search term." : "Contacts will appear here once they interact on WhatsApp."}
          </p>
        </div>
      ) : (
